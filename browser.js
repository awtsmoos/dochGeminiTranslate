...
const apiKey = "AIzaSyBMBFQfTQKFtcx4RxEdwii_VdFllVxLERo"; // <<< INSECURE!
...
async function doVolume(volume, curVol) {
    ...
    for(var key of keys) {
      ...
      //  continue
        var ai = await aiifySicha(actualSicha, curVol, actualSicha.Page )
...
async function d() {
   ...
   var sortedArray = Object.entries(sorted)
    ...
    for(var volume of sortedArray) {
        ...
        var done = doVolume(volumeObj, curVol);
